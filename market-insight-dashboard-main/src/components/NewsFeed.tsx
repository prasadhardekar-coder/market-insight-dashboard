  ...
  return (
    <div className="glass-card p-4 h-full flex flex-col">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Financial News</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-2 scrollbar-thin">
        {loading ? (
          ...
          news.map((item) => (
            <div
              key={item.id}
              className="bg-secondary rounded-lg p-3 hover:bg-muted transition-colors cursor-pointer group animate-slide-up"
            >
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="text-xs text-muted-foreground font-mono">{timeAgo(item.timestamp)}</span>
                <span className={`ml-auto text-xs px-2 py-0.5 rounded-full border ${sentimentStyles[item.sentiment]}`}>
                  {item.sentiment}
                </span>
              </div>
            </div>
